import React from 'react';
import { X, BarChart3, ShoppingCart } from 'lucide-react';

function D({ label, v }) {
  return (
    <div className="min-w-0">
      <p className="text-slate-400 text-[8px] leading-tight truncate">{label}</p>
      <p className="font-semibold text-white text-[11px] leading-tight truncate">{v}</p>
    </div>
  );
}

const riskRewardPct = (s) => {
  const e = s.entry || s.currentPrice, sl = s.stopLoss, t = s.target;
  if (!e || !sl || !t || sl >= e || t <= e) return null;
  return { risk: ((e - sl) / e * 100).toFixed(1), reward: ((t - e) / e * 100).toFixed(1) };
};
const fmtReason = (s) => s.reason ? s.reason.replace(/\s*\|\s*R:R\s*1:[\d.]+/i, '').replace(/R:R\s*1:[\d.]+\s*\|\s*/i, '') : s.reason;
const allocation = (s) => {
  const e = s.entry || s.currentPrice, q = s.recommendedQty || 0;
  return e && q ? Math.round(e * q) : null;
};

// Fullscreen detail sheet — sits below ChartModal (z-40 < z-50) so tapping
// "View Chart" can layer the chart on top without losing this modal's state.
// Compact by design: header, metrics, AI/screener detail and footer actions
// are all sized to fit one screen without scrolling on a typical phone.
export default function StockDetailModal({ stock, open, onClose, onBuy, onViewChart }) {
  if (!open || !stock) return null;

  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-slate-900">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-700 bg-slate-800 flex-shrink-0">
        <div className="min-w-0">
          <h3 className="text-base font-bold text-white leading-tight truncate">{stock.symbol}</h3>
          <p className="text-slate-400 text-[10px] truncate">{stock.company}</p>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className="text-right">
            <p className="text-sm font-bold text-white">₹{stock.currentPrice}</p>
            <p className={`text-[10px] font-semibold ${stock.change >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {stock.change >= 0 ? '+' : ''}{stock.change?.toFixed(2)}%
            </p>
          </div>
          <button onClick={onClose} className="p-1.5 text-slate-300 hover:text-white bg-slate-700 rounded-lg flex-shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>
      </div>

      {/* Content — sized to fit without scrolling; overflow-y-auto remains
          as a safety net for unusually long reason text / many tags */}
      <div className="flex-1 overflow-y-auto px-3 py-2 min-h-0">
        <div className="bg-slate-700 rounded-lg p-2.5">
          {/* Key Metrics */}
          <div className="grid grid-cols-3 gap-x-2 gap-y-1.5">
            <div>
              <p className="text-[8px] text-slate-400">Target</p>
              <p className="text-sm font-bold text-green-400">₹{stock.target}</p>
            </div>
            <div>
              <p className="text-[8px] text-slate-400">Stop Loss</p>
              <p className="text-sm font-bold text-red-400">₹{stock.stopLoss}</p>
            </div>
            <div>
              <p className="text-[8px] text-slate-400">Confidence</p>
              <p className="text-sm font-bold text-blue-400">{stock.confidence}%</p>
            </div>
            <div>
              <p className="text-[8px] text-slate-400">Upside</p>
              <p className="text-sm font-bold text-purple-400">
                {(((stock.target - stock.currentPrice) / stock.currentPrice) * 100).toFixed(1)}%
              </p>
            </div>
            <div>
              <p className="text-[8px] text-slate-400">Risk:Reward</p>
              <p className="text-sm font-bold text-blue-300">
                {(() => { const rr = riskRewardPct(stock); return rr ? `${rr.risk}:${rr.reward}` : (stock.rrRatio != null ? `1:${stock.rrRatio}` : '—'); })()}
              </p>
            </div>
            <div>
              <p className="text-[8px] text-slate-400">Total Risk</p>
              <p className="text-sm font-bold text-red-300">
                {stock.riskPerShare != null && stock.recommendedQty
                  ? `₹${Math.round(stock.riskPerShare * stock.recommendedQty).toLocaleString('en-IN')}`
                  : '—'}
              </p>
            </div>
            <div className="col-span-3">
              <p className="text-[8px] text-slate-400">Allocation</p>
              <p className="text-sm font-bold text-amber-300">
                {allocation(stock) != null ? `₹${allocation(stock).toLocaleString('en-IN')}` : '—'}
                <span className="text-[9px] text-slate-500 font-normal ml-1">({stock.recommendedQty} × ₹{stock.entry ?? stock.currentPrice})</span>
              </p>
            </div>
          </div>

          <p className="mt-2 text-[10px] text-slate-300 leading-snug line-clamp-2">
            <strong>Reason:</strong> {fmtReason(stock)}
          </p>

          {/* AI chart analysis (Gemini v3) */}
          {stock.aiRatings && (
            <div className="mt-2 bg-emerald-900/15 border border-emerald-800/40 rounded-lg p-2">
              <p className="text-[9px] font-bold tracking-widest text-emerald-300 mb-1">
                🤖 AI ANALYSIS <span className="text-slate-500 font-normal">(#{stock.aiRank ?? '—'})</span>
              </p>
              <div className="grid grid-cols-3 gap-x-2 gap-y-1">
                <D label="Volume" v={stock.aiRatings.volumePattern ?? '—'} />
                <D label="Base" v={stock.aiRatings.baseStructure ?? '—'} />
                <D label="Pullback" v={stock.aiRatings.pullbackDepth ?? '—'} />
                <D label="Base Type" v={stock.aiBaseType ?? '—'} />
                <D label="Extended?" v={stock.aiExtended ? '⚠️ Yes' : 'No'} />
                <D label="AI Reco" v={stock.aiRecommendation ?? '—'} />
              </div>
              {stock.aiVerdict && (
                <p className="text-[9px] text-emerald-200/90 mt-1 italic line-clamp-2">"{stock.aiVerdict}"</p>
              )}
            </div>
          )}

          {/* Full screener detail */}
          {stock.entryType && (
            <div className="mt-2 pt-2 border-t border-slate-600">
              <div className="flex flex-wrap gap-1 mb-1.5 text-[9px]">
                {stock.regime && <span className="px-1.5 py-0.5 rounded bg-purple-900/60 text-purple-300">🌍 {stock.regime}</span>}
                <span className="px-1.5 py-0.5 rounded bg-blue-900/60 text-blue-300">🎯 {stock.entryType}</span>
                {stock.signalBarDate && <span className="px-1.5 py-0.5 rounded bg-slate-600 text-slate-200">📅 {stock.signalBarDate}</span>}
              </div>
              <div className="grid grid-cols-3 gap-x-2 gap-y-1">
                <D label="Entry" v={`₹${stock.entry ?? '—'}`} />
                <D label="Qty" v={`${stock.recommendedQty}${stock.baseStage != null ? ` (st${stock.baseStage})` : ''}`} />
                <D label="Risk/Sh" v={stock.riskPerShare != null ? `₹${stock.riskPerShare}` : '—'} />
                <D label="Base Qual" v={stock.baseQuality != null ? stock.baseQuality.toFixed(2) : '—'} />
                <D label="Liquidity" v={stock.liquidityCr != null ? `₹${stock.liquidityCr}cr` : '—'} />
                <D label="IFP" v={stock.ifp ?? '—'} />
              </div>
            </div>
          )}
        </div>
      </div>

      {/* Sticky footer actions — bottom-anchored, full-width, thumb-friendly
          for one-handed use. Buy is last (closest to the natural right-thumb
          rest position) since it's the primary action. */}
      <div className="flex-shrink-0 px-3 py-2 border-t border-slate-700 bg-slate-800 space-y-1.5">
        <button
          onClick={onViewChart}
          className="w-full bg-slate-700 active:bg-slate-600 rounded-lg py-2.5 flex items-center justify-center gap-2 font-semibold text-sm text-blue-400 border border-slate-600"
        >
          <BarChart3 className="w-4 h-4" />
          View Chart
        </button>

        {stock.owned ? (
          <div className="w-full bg-slate-700 border border-purple-600/50 text-purple-200 font-semibold py-2.5 px-4 rounded-lg text-center text-xs">
            {stock.heldQty > 0
              ? `Holding ${stock.heldQty} — manage from Today tab`
              : stock.positionQty > 0
                ? `Bought today (${stock.positionQty}) — manage from Today tab`
                : 'BUY order already resting on Dhan'}
          </div>
        ) : (
          <button
            onClick={() => onBuy(stock)}
            className="w-full bg-gradient-to-r from-green-500 to-green-600 active:from-green-600 active:to-green-700 text-white font-bold py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 transition-all text-sm"
          >
            <ShoppingCart className="w-4 h-4" />
            Buy {stock.symbol}
          </button>
        )}
      </div>
    </div>
  );
}
